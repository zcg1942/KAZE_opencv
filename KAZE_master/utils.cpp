...
/* ************************************************************************* */
cv::Mat compute_inliers_ransac(const std::vector<cv::Point2f> &matches, std::vector<cv::Point2f> &inliers,
  ...
  if (use_fund == true)
    ...
    //float a = H.at<float>(0, 0);//从这里访问像素值也出错
...
bool read_homography(const string& hFile, cv::Mat& H1toN) {
  ...
  ifstream pf;
  pf.open(filename.c_str(), std::ifstream::in);//文件以输入方式打开(文件数据输入到内存)
